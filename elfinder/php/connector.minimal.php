...
define('ELFINDER_DROPBOX_CONSUMERKEY',    'ly8s3l4ioffy08j');
define('ELFINDER_DROPBOX_CONSUMERSECRET', 'ncw5tzes50bih9z');
...
$opts = array(
	...
	'roots' => array(
		array(
			...
			'path'          => './../files/'.$_SESSION['path'][0]['path'],                 // path to files (REQUIRED)
			'URL'           => './../files/', // URL to files (REQUIRED)
			'uploadDenyuploadDeny'    => array('all'),                // All Mimetypes not allowed to upload
...
foreach ($_SESSION['path'] AS $p) {
	...
	$opts['roots'][$i]['path'] = './../files/' . $p['path'];
	$opts['roots'][$i]['URL'] = './../files/' . $p['path'];
