...
class RunScheduledAutomatismes extends Command
{
    ...
    {
        // Console context: no authenticated user, so the tenant scope is off and
        // this returns every société's rules. Each rule is then processed inside
        // its own société context so interventions, runs and messages never cross
        // tenant boundaries.
        $rules = Automatisme::where('actif', true)->where('evenement', 'rendez_vous')->get();
        $sent = 0;

        foreach ($rules as $rule) {
            app(Tenancy::class)->forSociety($rule->society_id, function () use ($rule, $runner, &$sent) {
