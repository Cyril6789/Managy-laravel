        ...
        <div class="fixed inset-0 z-[90] flex items-start justify-center overflow-y-auto bg-black/60 p-4 py-8" wire:key="manual-invoice-modal" x-on:keydown.escape.window="$wire.set('show', false)">
            <div class="max-h-[94vh] w-full max-w-[95rem] overflow-y-auto rounded-xl bg-white p-6 shadow-2xl dark:bg-gray-900">
                ...
                <div class="mt-6 rounded-xl border border-gray-200 p-4 dark:border-gray-700">
                    <div class="grid grid-cols-1 gap-3 md:grid-cols-4 {{ $vatEnabled ? 'xl:grid-cols-[minmax(165px,1.25fr)_minmax(190px,1.5fr)_64px_105px_70px_105px_80px_188px]' : 'xl:grid-cols-[minmax(190px,1.3fr)_minmax(230px,1.6fr)_70px_120px_80px_188px]' }}">
                        ...
                        <x-field label="Désignation"><x-input wire:model="draft.description" /></x-field>
                        <x-field label="Qté"><x-input type="number" min="0.01" step="0.01" wire:model="draft.quantity" /></x-field>
                        <x-field label="Prix unitaire"><x-input type="number" min="0" step="0.01" wire:model="draft.unit_price" /></x-field>
                        <x-field label="Unité"><x-input wire:model="draft.unit" placeholder="u, h…" /></x-field>
                        @if($vatEnabled)
                            <x-field label="Prix saisi en">
                            ...
                            <x-field label="TVA (%)"><x-input type="number" min="0" max="100" step="0.01" wire:model="draft.vat_rate" /></x-field>
                        @endif
                ...
                <div class="mt-6 overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                    <table class="min-w-full text-sm">
                        <thead class="bg-gray-50 text-xs uppercase text-gray-500 dark:bg-gray-800"><tr><th class="px-3 py-2 text-left">Désignation</th><th class="px-3 py-2 text-right">Qté</th><th class="px-3 py-2 text-left">Unité</th><th class="px-3 py-2 text-right">PU HT</th>@if($vatEnabled)<th class="px-3 py-2 text-right">TVA</th>@endif<th class="px-3 py-2 text-right">Remise</th><th class="px-3 py-2 text-right">Total HT</th><th></th></tr></thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800" x-data="{ dragging: null }">
                            @forelse ($lines as $index => $line)
                                ...
                                <tr wire:key="manual-line-{{ $index }}" x-on:dragover.prevent x-on:drop.prevent="$wire.moveLine(dragging, {{ $index }}); dragging = null" class="transition hover:bg-gray-50 dark:hover:bg-gray-800/40">
                                    ...
                                    @if($vatEnabled)<td class="w-24 px-3 py-2"><x-input type="number" min="0" max="100" step="0.01" wire:model.live.debounce.300ms="lines.{{ $index }}.vat_rate" /></td>@endif
                            ...
                            @empty
                                <tr><td colspan="{{ $vatEnabled ? 8 : 7 }}" class="px-4 py-8 text-center text-gray-400">Ajoutez au moins une ligne.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                ...
                <div class="mt-5 grid gap-5 lg:grid-cols-[1fr_420px]">
                    ...
                    <div class="rounded-xl bg-gray-50 p-4 text-sm dark:bg-gray-800/50">
                        ...
                        @endif
                        <div class="mt-2 flex justify-between border-t border-gray-200 pt-3 text-lg font-bold dark:border-gray-700"><span>{{ $vatEnabled ? 'Total TTC' : 'Total HT' }}</span><span>{{ number_format($vatEnabled ? $previewHt + $previewVat : $previewHt, 2, ',', ' ') }} €</span></div>
                    </div>
                </div>
