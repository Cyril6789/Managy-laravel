            ...
            <form wire:key="restitution-{{ $breakdown['prestations_net'] }}-{{ $breakdown['pieces_net'] }}-{{ $breakdown['deplacement'] }}"
                ...
                @if ($i->estDomicile() && ! $i->garantie)
                    <div x-show="!signing" class="space-y-4">
                        ...
                        @include('livewire.partials.maintenance-pack', [
                            'hasPack' => $maintenanceHasPack,
                            'totalHeures' => $totalHeures,
                            'solde' => $maintenanceSolde,
                            'hint' => 'Laissez à 0 pour que le client règle tout en argent (le pack ne sera pas débité).',
                        ])
