...
<body class="flex min-h-full items-center justify-center bg-gray-100 px-4 py-12 dark:bg-gray-950">
    <div class="w-full max-w-sm">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            @if ($step === 'password')
                {{-- Step 2: this address has no SSO → e-mail + password. --}}
                <p class="mb-5 text-sm text-gray-500">Saisissez votre mot de passe pour continuer.</p>
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-4">
                    ...
                    <x-field label="Mot de passe" name="password" required>
                        <x-input name="password" type="password" autocomplete="current-password" autofocus />
                    </x-field>
                ...
                <p class="mt-4 text-center text-sm">
                    <a href="{{ route('login', ['fresh' => 1]) }}" class="text-gray-500 hover:text-brand-600">← Changer d'adresse e-mail</a>
                </p>
            @else
                {{-- Step 1: e-mail only. The system then routes to SSO or password. --}}
                <p class="mb-5 text-sm text-gray-500">Entrez votre adresse e-mail pour continuer.</p>

                <form action="{{ route('login.identify') }}" method="POST" class="space-y-4">
                    @csrf
                    <x-field label="Adresse e-mail" name="email" required>
                        <x-input name="email" type="email" value="{{ old('email', $email) }}" autofocus autocomplete="email" />
                    </x-field>

                    <x-button type="submit" class="w-full">Continuer</x-button>
                </form>
            @endif
