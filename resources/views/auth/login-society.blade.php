...
<head>
    ...
    <title>Connexion · {{ $society->name }}</title>
    <script>
        (function () {
...
<body class="flex min-h-full items-center justify-center bg-gray-100 px-4 py-12 dark:bg-gray-950">
    <div class="w-full max-w-sm">
        <div class="mb-8 flex flex-col items-center gap-3">
            <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-600 text-2xl font-bold text-white">{{ mb_strtoupper(mb_substr($society->name, 0, 1)) }}</div>
            <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ $society->name }}</h1>
        </div>

        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <h2 class="mb-5 text-lg font-semibold text-gray-900 dark:text-gray-100">Connexion</h2>
            ...
            {{-- Password form (secours administrateur si le SSO est imposé) --}}
            <div x-data="{ showPassword: {{ $methods['password'] ? 'true' : 'false' }} }">
                @if ($methods['microsoft'] || $methods['google'])
                    <div class="space-y-2">
                        @if ($methods['microsoft'])
                            <form action="{{ route('sso.redirect', 'microsoft') }}" method="POST">
                                @csrf
                                <input type="hidden" name="society" value="{{ $society->slug }}">
                    ...
                    @if (! $methods['password'])
                        <p class="mt-4 text-center text-xs">
                            <button type="button" @click="showPassword = ! showPassword" class="text-gray-400 hover:text-brand-600">Connexion administrateur (mot de passe)</button>
                        </p>
                    @else
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-4" x-show="showPassword" @if (! $methods['password']) x-cloak @endif>
                    @csrf
                    <x-field label="Adresse e-mail" name="email" required>
                        <x-input name="email" type="email" value="{{ old('email', $email) }}" autocomplete="email" />
                    </x-field>
        ...
        <p class="mt-6 text-center text-xs text-gray-400">© {{ date('Y') }} {{ $society->name }}</p>
    </div>
    @livewireScripts
</body>
</html>
