...
function buttons_client($id_client, $client)
{
    ...
    if(right('customers', 1))
        $html = '<a href="./c'.$id_client.'" title="Fiche client"><i class="fa fa-user"></i></a>';
    if(right('inter', 2) AND AccesActivableModul('intervention_immo'))
        $html .= '<a href="./affaire-new-customer-'.$id_client.'" title="Créer une intervention"><i class="fa fa-plus"></i></a>';
    if(right('inter', 2) AND AccesActivableModul('intervention_holtzmann') OR AccesActivableModul('intervention_informatique'))
        $html .= '<a href="./intervention-new-customer-'.$id_client.'" title="Créer une intervention"><i class="fa fa-plus"></i></a>';
    if(right('calendar', 2) AND AccesActivableModul('calendar'))
        $html .= '<a href="javascript:void();" onclick="$(\'#modal_add_rdv\').modal(\'show\'); $(\'#client_add_rdv\').html(\''.$client.'\'); $(\'#id_client_add_rdv\').val(\''.$id_client.'\');" title="Créer un rendez-vous"><i class="fa fa-calendar-plus-o"></i></a>';
...
*/
            ...
            $html .= ' <a href="./c'.$id_client.'#tab_tab_4">('.$solde_display.' restants)</a>';
        }
    }
...
{
    $sql = 'SELECT clients.id AS id_c, clients.titre, clients.nom, clients.prenom, id_inter, prefix
            FROM interventions
            INNER JOIN clients
    ...
    while ($data = $db->Row()) {
        $result[$i] = array(
            ...
            'sub' => '<a href="./c'.$data->id_c.'">'.$data->titre . ' '.$data->nom.' '.$data->prenom,
        );
        $i++;
    }
...
//print_r($result);
if(is_file('../templates/v4/ajax/search.php'))
  include('../templates/v4/ajax/search.php');
else
