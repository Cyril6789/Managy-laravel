...
function buttons_client($id_client)
{
    $db = new MySQL();
    ...
    if(right('calendar', 2) AND AccesActivableModul('calendar'))
        $html .= '<a href="./rdv-new-customer-'.$id_client.'" title="Créer un rendez-vous"><img src="./templates/mango/img/icons/packs/fugue/16x16/calendar.png" width="20px"/></a>';

    $db->Query('SELECT id_inter, (SELECT COUNT(*) FROM prise_en_charge AS pec WHERE pec.id_intervention=interventions.id_inter AND  pec.compte_principal="'.$_SESSION['compte_principal'].'" ) AS nb_pec FROM interventions WHERE id_client = "'.$id_client.'" AND time_cloture="0" AND compte_principal="'.$_SESSION['compte_principal'].'"  ');
    ...
    if(AccesActivableModul('pack_maintenance') AND right('pack_maintenance', 2))
    {
...
if(!is_numeric($term))
{
    ...
    $sql .= ' compte_principal = "' . $_SESSION['compte_principal'] . '"  ';
    $db->Query($sql);
    $result = Array();
    $i = 0;
    ...
    while ($data = $db->Row()) {
        ...
        $result[$i] = array(
            'href' => './c' . $data->id,
            'title' => $data->titre . ' ' . $data->nom . ' ' . $data->prenom.' '.buttons_client($data->id),
        );
        $i++;
    }
...
else
{
    ...
    while ($data = $db->Row()) {
        $result[$i] = array(
            'href' => './i' . $term,
            'title' => 'Intervention N°'.$term.' de '.$data->titre . ' ' . $data->nom . ' ' . $data->prenom,
        );
        $i++;
    }
